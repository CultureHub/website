...
export default function ProjectsList({
  ...
  function getActiveTag(): {
    category: keyof ProjectFilters;
    label: string;
    displayTitle?: ProgramOption["displayTitle"];
  }[] {
    const tags: {
      category: keyof ProjectFilters;
      label: string;
    ...
    for (const cat of FILTER_CATEGORIES) {
      ...
      if (val) {
        const displayTitle =
          cat === "program"
            ? allPrograms.find((p) => p.slug.current === filters[cat])
                ?.displayTitle
            : undefined;
        tags.push({ category: cat, label: val, displayTitle });
      }
    }
    return tags;
  }
        ...
        {projects.map((project) => {
          ...
          return (
            <div
              ...
              onMouseUp={() => setRowState({ id: project._id, type: "hover" })}
              onTouchStart={() =>
                setRowState({ id: project._id, type: "pressed" })
              }
              onTouchEnd={() => setRowState(null)}
              style={
                {
              ...
              className="flex flex-row border-b border-black cursor-pointer"
            >
              <div className="w-[125px] relative overflow-hidden flex-shrink-0 self-stretch">
                {project.heroImage ? (
                  <SanityImage
              ...
              <div className="flex flex-col gap-6 py-4 pr-6 pl-2 min-w-0 flex-1">
                <div className="flex flex-row flex-wrap gap-x-[56px] gap-y-1">
                  <span
                ...
                <div className="flex flex-col gap-2.5">
                  <p
                    className="font-sans font-normal text-[20px] leading-tight tracking-[-0.03em]"
                    style={{ color: "var(--row-text)" }}
                  >
                    {project.title}
                  </p>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 align-top py-3">
                    <span
                      className="font-mono italic text-xs leading-tight"
                      style={{ color: "var(--row-text)" }}
                    >
                      {project.program.displayTitle ? (
                        <PortableText value={project.program.displayTitle} />
