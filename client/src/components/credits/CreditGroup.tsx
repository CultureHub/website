...
export default function CreditGroup({ group }: { group: CreditGroupData }) {
  return (
    <div className="flex flex-col gap-4">
      {group.name && (
        <div className="flex flex-row py-2.5 justify-start items-center border-t border-b border-ch-midnite gap-2.5">
          <div className="w-[11px] h-[11px] rounded-full bg-ch-midnite shrink-0" />
          <h4 className="font-milling text-xl font-bold">{group.name}</h4>
        </div>
      )}
      {group.description && (
