...
export default function ArtistsIndexPage() {
  ...
  useEffect(() => {
    client.fetch(MEDIUM_QUERY, {}, options).then(setMediumOptions);
  }, []);

  const [artists, setArtists] = useState<SanityDocument[] | null>(null);
  useEffect(() => {
    const filters =
      mediumFilter.length > 0 ? [`count(medium[@ in $mediums]) > 0`] : [];
    const query = ARTIST_QUERY(filters);
    client
      .fetch<SanityDocument[]>(query, { mediums: mediumFilter }, options)
      .then(setArtists);
  }, [mediumFilter]);
